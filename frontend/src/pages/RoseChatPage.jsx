...
export default function RoseChatPage() {
    ...
    try {
      ...
      if (response.ok) {
        ...
        appendMessage(conversationId, {
          role: "model",
          text: data?.message || "Erro ao contactar a Rose.",
        });
      }
    } catch (_error) {
      appendMessage(conversationId, {
  ...
  return (
    <section className="h-[calc(100vh-7.5rem)] w-full max-w-none flex flex-col">
      <div className="mb-4">
        ...
        <div className="flex-1 min-h-0 rounded-2xl border border-slate-800 bg-slate-900/60 overflow-hidden flex flex-col">
          <div className="px-4 py-3 border-b border-slate-800 bg-slate-900 flex items-center gap-3">
            ...
            <button
              ...
              disabled={isLoading}
              className="ml-auto text-slate-400 hover:text-white hover:bg-slate-800 p-2 rounded-lg transition-colors disabled:opacity-50"
